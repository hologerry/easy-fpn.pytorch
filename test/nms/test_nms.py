...
class TestNMS(unittest.TestCase):
    ...
    def test_nms_empty(self):
        bboxes = torch.tensor([], dtype=torch.float).to(device)
        scores = torch.tensor([], dtype=torch.float).to(device)
        kept_indices = self._run_nms(bboxes, scores)
        self.assertEqual(len(kept_indices), 0)

    def test_nms_single(self):
        bboxes = torch.tensor([[5, 5, 10, 10]], dtype=torch.float).to(device)
        scores = torch.tensor([0.8], dtype=torch.float).to(device)
        kept_indices = self._run_nms(bboxes, scores)
        self.assertEqual(len(kept_indices), 1)
        self.assertListEqual(kept_indices.tolist(), [0])

    def test_nms_small(self):
        bboxes = torch.tensor([[5, 5, 10, 10], [5, 5, 10, 10], [5, 5, 30, 30]], dtype=torch.float).to(device)
        scores = torch.tensor([0.6, 0.9, 0.4], dtype=torch.float).to(device)
        kept_indices = self._run_nms(bboxes, scores)
    ...
    def test_nms_large(self):
        ...
        detections = torch.tensor(detections, dtype=torch.float).to(device)
...
if __name__ == '__main__':
    ...
    torch.tensor([]).to(device)  # dummy for initializing GPU
    unittest.main()
